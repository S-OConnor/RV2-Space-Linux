#!/usr/bin/env bash
# Build the development image (passwordless root login, debug tools).
set -euo pipefail
...
echo "note: artifacts will land in build/tmp/deploy/images/orangepi-rv2-mainline/"
exec "${KAS_CMD[@]}" build kas/rv2-dev.yml "$@"   # extra args pass through (e.g. --update)
